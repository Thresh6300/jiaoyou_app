package com.qiqi.jiaoyou_app.jPush.utils;
...
public class JpushUtil {
    ...
    private final static String APPKEY = "32f99e83c59eb73f17faeab4"; // "这是你的appkey"

    private final static String MASTERSECRET = "49cabe28bbacc0a08f9f5536"; //"这是你的mastersecret"
     ...
     */
    public static int sendToRegistrationIds( String notification_title, String msg_title, String msg_content, Map<String, String> extrasparams,String... registrationIds) {
	  int result = 0;
	  try {
		PushPayload pushPayload =  JpushUtil.buildPushObject_all_registrationId_alertWithTitle(notification_title, msg_title, msg_content, extrasparams,registrationIds);
    ...
    public static int sendToBieMing(String bieming, String notification_title, String msg_title, String msg_content, Map<String, String> extrasparams) {
        int result = 0;
        try {
            PushPayload pushPayload =  com.qiqi.jiaoyou_app.jPush.utils.JpushUtil.buildPushObject_all_BieMing_alertWithTitle(bieming, notification_title, msg_title, msg_content, extrasparams);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            if (pushResult.getResponseCode() == 200) {
                result = 1;
            }
    ...
    public static int sendToAllIos(String notification_title, String msg_title, String msg_content, String extrasparam) {
        int result = 0;
        try {
            PushPayload pushPayload = com.qiqi.jiaoyou_app.jPush.utils.JpushUtil.buildPushObject_ios_all_alertWithTitle(notification_title, msg_title, msg_content, extrasparam);
            PushResult pushResult = jPushClient.sendPush(pushPayload);
            if (pushResult.getResponseCode() == 200) {
                result = 1;
            }
...
//                                .addExtras(extrasparams)
     ...
     * @return
     */
    private static PushPayload buildPushObject_all_registrationId_alertWithTitle(String notification_title, String msg_title, String msg_content, Map<String, String> extrasparams, String... registrationIds) {
	  return PushPayload.newBuilder()
			//指定要推送的平台，all代表当前应用配置了的所有平台，也可以传android等具体平台
			.setPlatform(Platform.all())
			//指定推送的接收对象，all代表所有人，也可以指定已经设置成功的tag或alias或该应应用客户端调用接口获取到的registration id
